<?php
  ...
  class Config {
    ...
    /* 
    Revo Node Manager (RNM) password for login. You should additionally change the name of
    RNM folder to something unique, if accessible via the web. 
    Leave empty to disable it (only do this on a local setup that can't be reached from the outside).
     */
    const PASSWORD = "LOGIN-PASSWORD";
    /* 
    IP that can access RNM:
      "": any IP can access RNM
      "localhost": only localhost (IPv4/v6) can access RNM)
      "84.12.32.297": localhost and the specific IP (e.g. 84.12.32.297) can access RNM
    */
    const ACCESS_IP = "";

    /*
    IP of the Revo Core RPC Server, usually localhost. Be careful with remote servers. The connection is not encrypted by default.
    DOCKER:
    ...
    */
    ...
    // RPC port, usually 8332 if not changed in revo.conf
    const RPC_PORT = "8332";
    // RPC username / rpcauth specified in revo.conf
    const RPC_USER = "USERNAME";
    // RPC password / rpcauth specified in revo.conf
