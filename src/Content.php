...
function createBlocksContent(){
	...
	$blockHash = $revod->getbestblockhash();

	for($i = 0; $i < Config::DISPLAY_BLOCKS; $i++){
		$block = $revod->getblock($blockHash);
		...
		$coinbaseTx = $revod->getrawtransaction($block["tx"][0], 1, $block["hash"]);
...
function createForksContent(){
	...
	foreach($forks as $fork){
		...
		if($fork["status"] != "headers-only" AND $fork["status"] != "unknown"){
			$block = $revod->getblock($fork["hash"]);
