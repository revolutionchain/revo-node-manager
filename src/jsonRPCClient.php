...
/*
EasyRevo-PHP
A simple class for making calls to Revo's API using PHP.
https://github.com/aceat64/EasyRevo-PHP
    ...
    public function __call($method, $params)
    {
        ...
        if($this->status === 500 && $this->response['error']) {
          $this->error = $this->response['error']['message'];
            ...
            // If revod didn't return a nice error message, we need to make our own
            switch ($this->status) {
                case 401:
                    $this->error = 'Invalid RPC credentials';
                    break;
                case 0:
                    $this->error = 'Revo Core not reachable: '.$curl_error;
                    break;
                case 404:
