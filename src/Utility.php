...
function getCleanClient($client){
	...
	}elseif(preg_match("/Revo Wallet:([0-9]\.[0-9]{1,2})/i",$client, $matches)){
		$client = "Revo Wallet ".$matches[1];
	}else{
...
function checkSPV($client){
	if (preg_match('/MultiBit|revoj|bread/i',$client)){
		return true;
	}else{
		return false;
	}
    ...
    // Checks if we know about the oldest peer, if not we assume that we don't known any peer
		foreach($arrayPeers as $key => $peer){
			if($oldestPeerIp == $peer[0]){
				$delete = true;
				// Either revod was restarted or peer reconnected. Since peer is the oldest, all other peers we known disconnected
				if($oldestPeerId != $key){
					$delete = false;
				}
				break;
