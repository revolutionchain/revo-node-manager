...
$revod = new jsonRPCClient(Config::RPC_USER, Config::RPC_PASSWORD, $rpcIp, $rpcPort);
...
// Peers Page   
	...
	if(isset($_GET['c']) && $_GET['t'] == $_SESSION["csfrToken"]){
		...
		if($_GET['c'] == "ban"){
			...
			if($err == 0){
				try {
					$result = $revod->setban($ip, "add", $bantime);
					// Sleep necessary otherwise peer is still returned by revo core
					sleep(1);
		...
		// Disconnect Command
			...
			if(preg_match("/^(\[{0,1}[0-9a-z:\.]{7,39}\]{0,1}:[0-9]{1,5})$/", $_GET['ip'], $match)) {
				$ip = $match[1];
				try {
					$result = $revod->disconnectnode($ip);
					// Sleep necessary otherwise peer is still returned by revo core
					sleep(1);
			...
			try {
				...
				foreach($banlist as $ban){
					...
					if(checkIpBanList($ban[0]) && $timestamp !== FALSE){
						$result = $revod->setban($ban[0], "add", $timestamp, true);
						$i++;
					}
				}
				if($i !== 0){
